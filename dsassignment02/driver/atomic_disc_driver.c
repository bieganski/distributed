...
#define SECTORS_NUMBER (1 << 12)
#define ATDD_REQUESTS_BUF_SIZE_BITS 8
// Driver awaits execution of command after it is sent. It could send
// multiple commands at once and complete block device requests as
// repsonses come back.
#define ATDD_CLIENT_SEND_AHEAD ((uint8_t) 1)
#define ATDD_REQUESTS_BUF_SIZE (1 << ATDD_REQUESTS_BUF_SIZE_BITS)
// Setting thread count to higher values seems not to work properly...
#define ATDD_THREADS_COUNT (1 << 0)
...
struct atdd_ktread_data *atdd_kthread_data_create(struct atdd_client_config *shared_conf, int id)
{
  ...
  data->requests_buf = vmalloc(ATDD_REQUESTS_BUF_SIZE * sizeof(struct request *));
...
static int atdd_finish_all_completed_requests(struct atdd_ktread_data *data)
{
  ...
  while ((err = atdd_fetch_command(data->atdd_c, &cmd_r)) > 0) {
    rq = data->requests_buf[data->buf_completed_pos++];
    ...
    data->request_ident_counter += ATDD_THREADS_COUNT;
  }
...
static int atdd_send_accepted_requests(struct atdd_ktread_data *data)
{
  ...
  BUG_ON(cmds_to_send > 1);

  while (cmds_to_send--) {
    if ((err = atdd_send_command(
        data->atdd_c, data->requests_buf[data->buf_tcp_forwarded_pos],
        data->request_ident_counter + ATDD_THREADS_COUNT *
          (data->buf_tcp_forwarded_pos - data->buf_completed_pos))) < 0)
      return err;
    ...
    struct request *rq)
{
  ...
  relay->requests_buf[relay->buf_accepted_pos++] = rq;
